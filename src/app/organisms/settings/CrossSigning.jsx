...
const failedDialog = () => {
  const renderFailure = (requestClose) => (
    <div className="cross-signing__failure">
      ...
      <Text weight="medium"><Trans i18nKey="Organisms.CrossSigning.setup_failed" /></Text>
      <Button onClick={requestClose}><Trans i18nKey="common.close" /></Button>
    </div>
  );

  openReusableDialog(
    <Text variant="s1" weight="medium"><Trans i18nKey="Organisms.CrossSigning.setup" /></Text>,
    renderFailure,
  );
};
...
const securityKeyDialog = (key) => {
  ...
  const renderSecurityKey = () => (
    <div className="cross-signing__key">
      <Text weight="medium"><Trans i18nKey="Organisms.CrossSigning.save_security_key_message" /></Text>
      <Text className="cross-signing__key-text">
        {key.encodedPrivateKey}
      </Text>
      <div className="cross-signing__key-btn">
        <Button variant="primary" onClick={() => copyKey(key)}><Trans i18nKey="common.copy" /></Button>
        <Button onClick={() => downloadKey(key)}><Trans i18nKey="common.download" /></Button>
      </div>
    </div>
  );
  ...
  openReusableDialog(
    <Text variant="s1" weight="medium"><Trans i18nKey="Organisms.CrossSigning.security_key_dialog_title" /></Text>,
    () => renderSecurityKey(),
  );
};
...
function CrossSigningSetup() {
  ...
  const setup = async (securityPhrase = undefined) => {
    ...
    const authUploadDeviceSigningKeys = async (makeRequest) => {
      const isDone = await authRequest('Quersignaturen einrichten', async (auth) => {
...
const setupDialog = () => {
  openReusableDialog(
    <Text variant="s1" weight="medium"><Trans i18nKey="Organisms.CrossSigning.setup_cross_signing" /></Text>,
    () => <CrossSigningSetup />,
  );
};
...
const resetDialog = () => {
  openReusableDialog(
    <Text variant="s1" weight="medium"><Trans i18nKey="Organisms.CrossSigning.reset_cross_signing" /></Text>,
    () => <CrossSigningReset />,
  );
};
